		...
		enum jesd204_state_op_reason reason)
{
	...
	no_os_clk_set_rate(phy->clks[RX_SAMPL_CLK], phy->rx_iqRate_kHz * 1000);
	no_os_clk_set_rate(phy->clks[TX_SAMPL_CLK], phy->tx_iqRate_kHz * 1000);
...
int adrv9025_setup(struct adrv9025_rf_phy *phy)
{
	...
	strncpy(phy->platformFiles.streamImageFile,
