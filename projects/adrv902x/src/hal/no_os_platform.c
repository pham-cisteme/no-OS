...
int32_t no_os_HwOpen(void *devHalCfg)
{
	...
	struct no_os_gpio_init_param gip_gpio_reset_n;
	...
	gip_gpio_reset_n.extra = &gip_extra;
	gip_gpio_reset_n.platform_ops = &xil_gpio_ops;
			     ...
			     uint32_t pageIndex, uint32_t pageSize, uint8_t *rdBuff)
{
	...
	if (!strcmp(ImagePath, "ADRV9025_FW.bin")) {
		...
		if ((pageIndex * pageSize) > sizeof(
			    stream_image_6E3E00EFB74FE7D465FA88A171B81B8F_bin))
			return -EINVAL;
		bin = stream_image_6E3E00EFB74FE7D465FA88A171B81B8F_bin;
	} else
...
FILE* __fopen(const char * filename, const char *mode)
{
	...
	if (!strcmp(filename, "ActiveUseCase.profile")) {
		...
		profile.end = profile.start + strlen(profile.data);
			   ...
			   "stream_image_6E3E00EFB74FE7D465FA88A171B81B8F.bin")) {
		length = sizeof(stream_image_6E3E00EFB74FE7D465FA88A171B81B8F_bin);
		temp = (unsigned char *)no_os_calloc(length, sizeof(unsigned char));
		memcpy(temp, stream_image_6E3E00EFB74FE7D465FA88A171B81B8F_bin, length);
		profile.data = temp;
		profile.start = profile.ptr = profile.data;
